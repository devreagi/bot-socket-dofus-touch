...
export default class Storage {
  ...
  public async UpdateStorageInventoryContentMessage(
    ...
    if (message.objects.length > 0) {
      ...
      for (const obj of message.objects) {
        const oe = objects.find(f => f.id === obj.objectGID);
  ...
  public async UpdateStorageObjectUpdateMessage(
    ...
    if (obj === undefined) {
      ...
      this.objects.Add(await ObjectEntry.setup(message.object, data[0]));
    } else {
      // Needs to be updated
  ...
  public async UpdateStorageObjectsUpdateMessage(
    ...
    for (const item of message.objectList) {
      ...
      if (obj === undefined) {
        ...
        this.objects.Add(await ObjectEntry.setup(item, data[0]));
      } else {
        // Needs to be updated
